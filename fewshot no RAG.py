...
nlp = spacy.blank("en")  # Start with blank English pipeline
...
def get_pdf_files(directory):
    ...
    return pdf_files
pdf_files = get_pdf_files("/content/Untitled Folder")
listpara=[]
for pdf in pdf_files:
    paragraphs = extract_paragraphs_from_pdf(pdf_path=pdf)
    listpara.append(paragraphs)

# leave the ones with highest confidence above 0.2
thelist=[]
paragraphe=[]
for paraa in listpara:
  for para in paraa:
    doc = nlp(para)
    if max(doc.cats.values()) > 0.2:
        max_key = max(doc.cats, key=doc.cats.get)
        thelist.append({max_key : doc.cats[max_key]})
        paragraphe.append({
                    'Paragraph': para,
                    'Category': max_key,
                    'Confidence': doc.cats[max_key]
                })
# 3. Split into sentences
    
# Create DataFrame and save to Excel
df = pd.DataFrame(paragraphe)
df.to_excel("classified_paragraphs.xlsx", index=False)

print(f"Successfully saved {len(df)} paragraphs to 'classified_paragraphs.xlsx'")
